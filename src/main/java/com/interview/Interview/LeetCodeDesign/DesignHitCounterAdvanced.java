...
/*
Case 2: Unordered Input (Harder)

If hits come out of order, or multiple hits have same timestamp, the queue approach breaks.

🔧 Solution: Use TreeMap<Integer, Integer>
	•	Key = timestamp
	•	Value = count of hits at that timestamp
	•	On hit(t), just increment count at timestamp t
	•	On getHits(t), sum all hits in the range [t-299, t]
 */
public class DesignHitCounterAdvanced {
    ...
    public int getHits1(int timestamp) {
        int total_hits = 0;
        int threshold = timestamp - 300 + 1;
        System.out.println("size : "+map.size());
        for (Map.Entry<Integer, Integer> entry : map.tailMap(threshold).entrySet()) {
            total_hits += entry.getValue();
        }
    ...
    //-----------------------------------------------------------------------------------
        ...
        d.hit(2);
        d.hit(3);
        System.out.println(d.getHits(4)); // expect 4 -> {1->1,2->2,3->1}
        d.hit(300);
        System.out.println(d.getHits(300)); // expect 5 -> {1->1,2->2,3->1,300->1}
        System.out.println(d.getHits(301)); //expect 4 -> {2->2,3->1,300->1}
        d.hit(303); //  {1->1,2->2,3->1,300->1,303->1}
        System.out.println(d.getHits(303)); // expect 2 -> {300->1,303->1}
    }
}
