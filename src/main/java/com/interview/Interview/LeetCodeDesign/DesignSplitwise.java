...
class ExpenseSharingApp {
    ...
    public void addExpense(Expense expense) {
        /*
            1. Equal Split
                Logic: Total amount is divided equally among all participants.
                       share = totalAmount / numOfUsers
                       Each user (except the payer) owes the payer this share.

                Example: If 1000 is spent by u1 for 4 users → each (u2, u3, u4) owes 250 to u1.
         */
        if (expense.expenseType.equals(ExpenseType.EQUAL)) {
        ...
        /*
            2. Exact Split
                Logic: Each participant’s share is explicitly provided.
                       The shares list contains the exact amounts each user should pay.
                       updateBalances(usersInvolved.get(i), paidBy, shares.get(i));

                Example: If 1250 spent by u1 with shares [370, 880] for u2 and u3, then
                u2 owes u1 → 370
                u3 owes u1 → 880
         */
        else if (expense.expenseType.equals(ExpenseType.EXACT)) {
        ...
        /*
            3. Percent Split
                Logic: Total is split based on given percentages.
                       share = (totalAmount * percent) / 100
                       Percentages must sum up to 100, else error.
                       Each user (except the payer) owes their percentage-based share.

                Example: If 1200 spent by u4 with [40, 20, 20, 20] for 4 users,
                u1 owes u4 → 480
                u2 owes u4 → 240
                u3 owes u4 → 240
         */
        else if (expense.expenseType.equals(ExpenseType.PERCENT)) {
